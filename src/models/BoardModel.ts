...
export class BoardModel {
    ...
    public initCheckers() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            for (let x = 0; x < 8; x += 2) {
                ...
                */
                if (i % 2 === 1) {
                    this.cells[whiteY - i][x + 1].checker = new CheckerModel(ColorsEnum.WHITE, PlayersEnum.PLAYER_ONE);
                    this.cells[blackY + i][x].checker = new CheckerModel(ColorsEnum.BLACK, PlayersEnum.PLAYER_TWO);
                } else {
                    this.cells[whiteY - i][x].checker = new CheckerModel(ColorsEnum.WHITE, PlayersEnum.PLAYER_ONE);
                    this.cells[blackY + i][x + 1].checker = new CheckerModel(ColorsEnum.BLACK, PlayersEnum.PLAYER_TWO);
                }
            }
        }
    }
    ...
    */
    public getAllMoves(player: PlayersEnum): TMoves {        
        ...
        this.cells.forEach((row) => {
            row.forEach((cell: CellModel) => {
                if (cell.checker !== null && cell.checker?.player === player) {
    ...
    public getMoves(cell: CellModel): TMoves {
        ...
        */
        ...
        if (cell.checker?.player === PlayersEnum.PLAYER_ONE && !cell.checker?.isKing) {
            if (!jumps.length) {
                singles = singles.concat(checkAdjacent(this.cells, upperY, leftX, rightX));
            }
        }

        if (cell.checker?.player === PlayersEnum.PLAYER_TWO && !cell.checker?.isKing) {
            if (!jumps.length) {
    ...
    */
    ...
    public hasMoves(player: PlayersEnum): boolean {
